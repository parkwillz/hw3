...
function selectQBWR($qbid) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("select t.team_id, w.widereceiver_id, w.widereceiver_name, w.widereceiver_adot, t.teamname from widereceiver w join team t on w.widereceiver_id = t.widereceiver_id where t.quarterback_id=?");
